...
Stochastic RSI + Trend + Momentum + Volume — same logic as the Binance bot
but adapted for MT5 forex data via MetaAPI.
...
# =====================================================
# RSI SERIES (Wilder's smoothing) — pure computation, stays sync
# =====================================================
...
async def calculate_stochastic_rsi(
    ...
    rsi_period=14, stoch_period=14, k_smooth=STOCH_K_SMOOTH, d_smooth=STOCH_D_SMOOTH,
):
    """
    Calculate Stochastic RSI from MT5 candle data via MetaAPI.
    ...
    print(f"[STOCHRSI] {symbol} | Requested={candles_needed} candles | Received={len(close_prices) if close_prices else 0}")
...
async def get_trade_signal_mt5(ctx, symbol, current_bid, support, resistance):
    ...
    try:
        ...
        if strong_buy_trigger:
            details["signal"] = "BUY"
            details["score"] = buy_score
        elif strong_sell_trigger:
            details["signal"] = "SELL"
            details["score"] = sell_score
        else:
            print(f"[MT5-SIGNAL] {symbol}: StochRSI K={k:.2f} — outside band (BUY: %K<={STOCH_RSI_BUY_LEVEL}, SELL: %K>={STOCH_RSI_SHORT_LEVEL}). No entry.")
            details["reasons"].append(
                f"No threshold hit (K={k:.2f}, need %K <= {STOCH_RSI_BUY_LEVEL} for BUY or %K >= {STOCH_RSI_SHORT_LEVEL} for SELL)"
            )
